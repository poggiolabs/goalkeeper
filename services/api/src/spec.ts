  ...
  paths: {
    ...
    [apiRoutes.authVerifyEmail.path]: {
      get: {
        ...
        description: "Consumes a single-use verification token and returns to the web application.",
        tags: ["Authentication"],
        parameters: [
          {
            name: "token",
          ...
          },
          {
            ...
            in: "query",
            required: false,
            description: "Same-origin application URL to open after verification.",
            schema: { type: "string", format: "uri" }
          }
        ],
        responses: {
          "302": {
            description: "Return to the application with the verification result.",
            headers: {
              Location: {
                description: "Application result location.",
                schema: { type: "string", format: "uri" }
              }
            }
          }
